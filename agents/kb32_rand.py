...
class Agent(nn.Module):
	...
	def tile_loss(self, x, Y):
		# feature names are shuffled to produce a useless taxonomy
		B, T, C = x.shape
		D = CONCEPT_DIM
		xv = x.reshape(B*T, C)
		y = torch.zeros(B, T, D).to(self.device)
		yv = y.view(B*T, D)
		yv[:,0] = (xv[:,SAPPHIRE]>0) | (xv[:,MELEE_2]>0) # drink
		yv[:,1] = (xv[:,CHEST]>0) | (xv[:,LADDER_UP]>0) | (xv[:,ENEMY_PROJECTILE_3]>0) | (xv[:,FURNACE]>0) # eat
		yv[:,2] = (xv[:,OUT_OF_BOUNDS]>0) | (xv[:,WATER]>0) | (xv[:,PASSIVE_2]>0) # archers
		yv[:,3] = (xv[:,CRAFTING_TABLE]>0) | (xv[:,LADDER_DOWN_BLOCKED]>0) | (xv[:,RIPE_PLANT]>0) # warriors
		yv[:,4] = (yv[:,2]>0) | (yv[:,3]>0) # all enemies
		yv[:,5] = (yv[:,4]>0) | (xv[:,LIGHT_LEVEL]<0.05) | (xv[:,LAVA]>0) | (xv[:,IRON]>0) # harmful
		yv[:,6] = (xv[:,PASSIVE_3]>0) | (xv[:,WOOD]>0)
		yv[:,7] = (xv[:,ENEMY_PROJECTILE_1]>0) | (xv[:,DIAMOND]>0) | (xv[:,RANGED_3]>0) | (xv[:,MELEE_3]>0) # no pickaxe
		yv[:,8] = (yv[:,7]>0) | (xv[:,PLANT]>0) | (xv[:,RANGED_1]>0) # wood tier
		yv[:,9] = (yv[:,8]>0) | (xv[:,WALL_MOSS]>0) # stone tier
		yv[:,10] = (yv[:,9]>0) | (xv[:,PASSIVE_1]>0) # iron tier
		yv[:,11] = (yv[:,10]>0) | (xv[:,WATER]>0) | (xv[:,CHEST]>0) # diamond tier
		yv[:,12] = (xv[:,STONE]>0) | (xv[:,MELEE_1]>0) | (xv[:,LADDER_DOWN]>0) # non-removable
		yv[:,13] = (xv[:,ENEMY_PROJECTILE_2]>0) | (xv[:,RUBY]>0) # can place block
		yv[:,14] = (xv[:,COAL]>0)
		yv[:,15] = (xv[:,RANGED_2]>0)
		yv[:,16] = (xv[:,LAVA]>0) | (xv[:,TREE]>0)
		yv[:,17] = (xv[:,LADDER_DOWN]>0)
		yv[:,18] = (xv[:,WALL]>0) # visibility mask
