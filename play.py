...
parser.add_argument("--steps", type=float, default=float('inf'))
...
while step < args.steps:
    ...
    while not done and step < args.steps:
        ...
        info = info.copy() * 0
        y = 20
        cv2.putText(info, f'ACTION', (10, y), FONT, SIZE, WHITE)
        y += 10
        ...
        y += 20; cv2.putText(info, f'DEPTH {depth}', (10, y), FONT, SIZE, WHITE)
        ...
        y += 20; cv2.putText(info, f'TOTAL {reward_sum:.2f}/226 ({100*reward_sum/226:.2f}%)', (10, y), FONT, SIZE, ACCENT)
        y = 20
        achv_count = int(achv_new.sum())
        cv2.putText(info, f'GOALS {achv_count}/{len(Achievement)}', (200+10, y), FONT, SIZE, WHITE)
        y += 10
    ...
    print(f'EP {episode} REWARD {reward_sum:.2f} ({100*reward_sum/226:.2f}%)')
    if score > hi_score:
        hi_score = score
        print(f'HI-SCORE! {datetime.now()}')
